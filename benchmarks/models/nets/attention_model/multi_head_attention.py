...
class Vanilla_AttentionScore(nn.Module):
    ...
    def forward(self, query, key, mask=None):
        d_k = query.size(-1)
        u_score = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)
        if mask is not None:
            mask = torch.tensor(mask).to(u_score.device).unsqueeze(0).unsqueeze(2)  # [1, B, 1, K]
...
class AttentionScore(nn.Module):
    ...
    def __init__(self, use_tanh=False, C=10.0):
        super(AttentionScore, self).__init__()
        self.use_tanh = use_tanh
        # 可学习的缩放因子（关键改进）
        self.scale = nn.Parameter(torch.tensor(1.0))  # 初始值为1
        # 使 C 始终为正数（使用 softplus 或 exp 约束）
        self._C = nn.Parameter(torch.tensor(math.log(C)))  # 取对数初始化，保证初值为 C
       
    @property
    def C(self):
        return torch.exp(self._C)  # 确保 C 始终为正数

    def forward(self, query, key, mask=None):
        d_k = query.size(-1)
        # 改进后的缩放计算（允许模型自动学习最佳缩放比例）
        u_score = torch.matmul(query, key.transpose(-2, -1)) * self.scale / math.sqrt(d_k)
        
        if self.use_tanh:
            logits = torch.tanh(u_score) * self.C  # 这里 C 一定是正数
        else:
            logits = u_score
